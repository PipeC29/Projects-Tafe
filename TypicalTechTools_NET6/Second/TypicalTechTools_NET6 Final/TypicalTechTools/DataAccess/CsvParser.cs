        ...
        public List<Comment> ParseComments()
        {
            ...
            */
            foreach (var comment in commentLines)
            {
                ...
                if (commentSections.Length >= 3 && int.TryParse(commentSections[0], out int commentId) &&
                    int.TryParse(commentSections[2], out int productCode))
                {
                    parsedComment.CommentId = commentId;
                    parsedComment.CommentText = commentSections[1];
                    parsedComment.ProductCode = productCode;
                }
                else
                {
                    Console.WriteLine($"Error parsing comment: {comment}");
                    continue; 
                }
